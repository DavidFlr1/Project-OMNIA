...
export class GoalManager {
  ...
  createGoal(
    ...
    this.memory.addEvent("goal_created", { goal });
  ...
  createMilestone(
    ...
    this.memory.addEvent("milestone_created", { 
   ...
   */
  updateGoal(goalId: string, updates: Partial<Omit<Goal, 'id' | 'createdAt' | 'milestones'>>): boolean {
    const goal = this.getGoal(goalId);
    if (!goal) {
    ...
    // If status is being updated to active, deactivate other goals
    if (updates.status === "active") {
      ...
      // Activate the first pending milestone if any
      const pendingMilestone = goal.milestones.find(m => m.status === "pending");
      if (pendingMilestone) {
        pendingMilestone.status = "active";
      }
    }
    
    // If status is being updated to completed, set completedAt and progress
    if (updates.status === "completed" && !goal.completedAt) {
      ...
      this.memory.addEvent("goal_completed", { goalId, goalName: goal.name });
      
      // If this was active, activate the next highest priority goal
        ...
        if (updates.status === "completed" && !milestone.completedAt) {
          ...
          if (milestone.status === "active") {
            ...
            if (nextMilestone) {
              ...
              this.updateGoal(goal.id, { status: "completed" });
            }
          }
        }
  ...
  /**
   * Gets the active milestone of the active goal
   */
  getActiveMilestone(): Milestone | null {
    const activeGoal = this.getActiveGoal();
    if (!activeGoal) return null;
    
    return activeGoal.milestones.find(m => m.status === "active") || null;
  }
