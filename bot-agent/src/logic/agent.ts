...
export interface AgentState {
  ...
  status: "idle" | "working" | "exploring" | "combat" | "gathering";
  health: number;
  food: number;
...
export class Agent {
  ...
  addGoal(goal: string): void {
    ...
    logger.info(`New goal added: ${goal}`);
  ...
  setCurrentGoal(goal: string): void {
    ...
    this.updateState({ currentGoal: goal });
    logger.info(`Current goal set to: ${goal}`);
  }

  completeCurrentGoal(): void {
    if (this.state.currentGoal) {
      logger.info(`Goal completed: ${this.state.currentGoal}`);
      this.memory.addEvent("goal_completed", { goal: this.state.currentGoal });

      // Remove completed goal from queue
      this.goals = this.goals.filter((g) => g !== this.state.currentGoal);
      ...
      if (this.goals.length > 0) {
        ...
        this.updateState({ currentGoal: null, status: "idle" });
      }
    }
  }
  ...
  handleLowHealth(): void {
    ...
    this.goals.unshift("find_shelter");

    if (this.state.currentGoal !== "find_shelter") {
      this.setCurrentGoal("find_shelter");
    }

    this.updateState({ status: "combat" });
  }
